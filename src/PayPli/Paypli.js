    ...
    return (
        <React.Fragment>
            <button
                onClick={onConnect}
                disabled={disable}
                className={`${props.style} ${disable && "btn-disabled"}`}
            >
                {btnLabel}
            </button><br />
            <small style={{ fontSize: 10, textAlign: "center" }}>1 {props.fiatcurrency} = {basePrice} {props.paymethod}</small>
        </React.Fragment>
    )
}
